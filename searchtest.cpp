...
const float MAX_BALL_AREA = 8000.0f;
...
public:
    ProcessResult process(Mat &input)
    {
        ...
        for (const auto &cnt : ballContours) {
            ...
            result.ballContours.push_back(cnt);
...
// ==============================================
// 【第一部分】你原本的主函数 (已重命名为 originalMain，内容完全未改)
// ==============================================
int originalMain() {
    cout << "========================================" << endl;
    cout << "  弹丸可视化优化版：保留原始形状 + 绿色外接圆" << endl;
    cout << "  运行模式：原始单线程处理" << endl;
    cout << "========================================" << endl;
...
// 【第二部分】新增的双线程测试支持代码
// ==============================================
#if RUN_DUAL_THREAD_TEST == 1

#define TEST_TARGET_FPS         50
#define TEST_QUEUE_MAX_SIZE     5
...
public:
    explicit ThreadSafeFrameQueue(size_t max_size) : max_size(max_size) {}

    bool push(const cv::Mat& frame) {
        std::unique_lock<std::mutex> lock(mtx);
        cv_not_full.wait(lock, [this]() {
            return frame_queue.size() < max_size || is_stop.load();
        });
        if (is_stop.load()) return false;
    ...
    bool pop(cv::Mat& out_frame) {
        std::unique_lock<std::mutex> lock(mtx);
        cv_not_empty.wait(lock, [this]() {
            return !frame_queue.empty() || is_stop.load();
        });

        if (is_stop.load() && frame_queue.empty()) return false;
    ...
    int get_drop_count() const { return drop_frame_count.load(); }

    // 【核心修复】对外提供is_stopped方法，返回队列是否已停止
    bool is_stopped() const { return is_stop.load(); }
};

struct PerformanceStats {
    std::atomic<int> total_produced_frames{0};
    std::atomic<int> total_processed_frames{0};
    std::atomic<int> invalid_packet_count{0};
    std::atomic<double> max_process_time_ms{0.0};
    std::atomic<double> min_process_time_ms{1000.0};

    std::mutex stats_mtx;
    std::vector<double> process_time_history;
    double total_process_time_sum = 0.0;
};

ThreadSafeFrameQueue g_test_frame_queue(TEST_QUEUE_MAX_SIZE);
PerformanceStats g_test_stats;
HeroCamCompressor g_test_compressor;
std::atomic<bool> g_test_capture_finished{false};

void testCaptureThreadFunc() {
    VideoCapture cap(VIDEO_FILE_PATH);
    if (!cap.isOpened()) {
        cerr << "[采集线程] 错误：无法打开视频文件" << endl;
        g_test_capture_finished.store(true);
        g_test_frame_queue.stop();
        return;
    }

    const int total_frames = (int)cap.get(CAP_PROP_FRAME_COUNT);
    cout << "[采集线程] 启动成功，目标帧率：" << TEST_TARGET_FPS << "Hz" << endl;

    auto next_frame_time = steady_clock::now();
    const auto frame_interval = duration_cast<steady_clock::duration>(duration<double, std::milli>(1000.0 / TEST_TARGET_FPS));
    ...
    while (true) {
        ...
        if (!cap.read(frame)) {
            cout << "[采集线程] 视频读取完毕" << endl;
            break;
        }

        g_test_frame_queue.push(frame);
        g_test_stats.total_produced_frames++;

        if (g_test_stats.total_produced_frames.load() % 100 == 0) {
            cout << "[采集线程] 已生产：" << g_test_stats.total_produced_frames.load() << " 帧" << endl;
        }
    }
...
void testProcessThreadFunc() {
    cout << "[处理线程] 启动成功" << endl;

    VideoCapture temp_cap(VIDEO_FILE_PATH);
    const int origWidth = temp_cap.isOpened() ? (int)temp_cap.get(CAP_PROP_FRAME_WIDTH) : ORIGINAL_SIZE.width;
    const int origHeight = temp_cap.isOpened() ? (int)temp_cap.get(CAP_PROP_FRAME_HEIGHT) : ORIGINAL_SIZE.height;
    temp_cap.release();

    Mat frame;
    uint8_t frame_seq = 0;
    vector<double> process_time_list;

    while (true) {
        if (!g_test_frame_queue.pop(frame)) {
            if (g_test_capture_finished.load() && g_test_frame_queue.is_stopped()) {
                break;
            }
            continue;
        }
        ...
        ProcessResult result = g_test_compressor.process(frame);
        result.packet.frame_seq = frame_seq;

        auto process_end = high_resolution_clock::now();
        double process_time_ms = duration_cast<duration<double, std::milli>>(process_end - process_start).count();
        process_time_list.push_back(process_time_ms);

        if (result.data_used_byte > TOTAL_PACKET_BYTE) {
            g_test_stats.invalid_packet_count++;
        }

        g_test_stats.total_processed_frames++;
        double current_max = g_test_stats.max_process_time_ms.load();
        if (process_time_ms > current_max) g_test_stats.max_process_time_ms.store(process_time_ms);
        double current_min = g_test_stats.min_process_time_ms.load();
        if (process_time_ms < current_min) g_test_stats.min_process_time_ms.store(process_time_ms);

        {
            std::lock_guard<std::mutex> lock(g_test_stats.stats_mtx);
            g_test_stats.process_time_history.push_back(process_time_ms);
            g_test_stats.total_process_time_sum += process_time_ms;
        }

        if (g_test_stats.total_processed_frames.load() % 100 == 0) {
            cout << "[处理线程] 已处理：" << g_test_stats.total_processed_frames.load() << " 帧" << endl;
        }
    }

    if (!process_time_list.empty()) {
        sort(process_time_list.begin(), process_time_list.end());
        double avg_time = accumulate(process_time_list.begin(), process_time_list.end(), 0.0) / process_time_list.size();
        double p99_time = process_time_list[(int)(process_time_list.size() * 0.99)];

        cout << "\n[处理线程] 核心性能数据：" << endl;
        cout << "----------------------------------------" << endl;
        cout << "  平均处理耗时：" << fixed << setprecision(2) << avg_time << "ms" << endl;
        cout << "  平均处理帧率：" << fixed << setprecision(1) << 1000.0/avg_time << "FPS" << endl;
        cout << "----------------------------------------" << endl;
    }
    cout << "[处理线程] 已退出" << endl;
}

int dualThreadTestMain() {
    cout << "========================================" << endl;
    cout << "  RoboMaster 双线程 50Hz 性能测试" << endl;
    cout << "  硬约束：单包≤" << TOTAL_PACKET_BYTE << "B" << endl;
    cout << "========================================" << endl;

    cout << "\n[主程序] 启动双线程..." << endl;
    std::thread capture_thread(testCaptureThreadFunc);
    std::thread process_thread(testProcessThreadFunc);

    capture_thread.join();
    process_thread.join();

    int total_produced = g_test_stats.total_produced_frames.load();
    int total_processed = g_test_stats.total_processed_frames.load();
    int drop_count = g_test_frame_queue.get_drop_count() + (total_produced - total_processed);
    double drop_rate = total_produced > 0 ? 100.0 * drop_count / total_produced : 0.0;
    
    double avg_process_fps = 0.0;
    {
        std::lock_guard<std::mutex> lock(g_test_stats.stats_mtx);
        if (!g_test_stats.process_time_history.empty()) {
            double avg_time_ms = g_test_stats.total_process_time_sum / g_test_stats.process_time_history.size();
            avg_process_fps = 1000.0 / avg_time_ms;
        }
    }

    cout << "\n\n";
    cout << "╔══════════════════════════════════════════════════════════════╗" << endl;
    cout << "║               双线程50Hz性能测试 最终报告                      ║" << endl;
    cout << "╠══════════════════════════════════════════════════════════════╣" << endl;
    cout << "║  目标帧率:        " << left << setw(15) << TEST_TARGET_FPS << " Hz                 ║" << endl;
    cout << "║  平均处理帧率:    " << left << fixed << setprecision(1) << setw(15) << avg_process_fps << " FPS                ║" << endl;
    cout << "║  50Hz适配结果:    " << left << (avg_process_fps >= TEST_TARGET_FPS ? "✅ 达标" : "❌ 不达标") << "                                   ║" << endl;
    cout << "║  总生产帧数:      " << left << setw(15) << total_produced << " 帧                  ║" << endl;
    cout << "║  总处理帧数:      " << left << setw(15) << total_processed << " 帧                  ║" << endl;
    cout << "║  总丢帧数:        " << left << setw(15) << drop_count << " 帧                  ║" << endl;
    cout << "╚══════════════════════════════════════════════════════════════╝" << endl;
